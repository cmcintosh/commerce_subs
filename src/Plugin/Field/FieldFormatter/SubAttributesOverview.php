...
/**
 * Plugin implementation of the 'commerce_sub_attributes_overview' formatter.
 *
 * @FieldFormatter(
 *   id = "commerce_sub_attributes_overview",
 *   label = @Translation("Sub attributes overview"),
 *   field_types = {
 *     "entity_reference",
 *   },
 * )
 */
class SubAttributesOverview extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  /**
   * Constructs a new SubAttributesOverview object.
   *
   ...
   *   The renderer.
   */
  public function __construct($plugin_id, $plugin_definition, FieldDefinitionInterface $field_definition, array $settings, $label, $view_mode, array $third_party_settings, EntityTypeManagerInterface $entity_type_manager, EntityDisplayRepositoryInterface $entity_display_repository, SubAttributeFieldManagerInterface $attribute_field_manager, RendererInterface $renderer) {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $sub_type_storage = $this->entityTypeManager->getStorage('commerce_sub_type');
    $attribute_storage = $this->entityTypeManager->getStorage('commerce_sub_attribute');
    /** @var \Drupal\commerce_sub\Entity\SubTypeInterface $sub_bundle */
    $sub_bundle = $sub_type_storage->load($this->fieldDefinition->getTargetBundle());
    $attribute_map = $this->attributeFieldManager->getFieldMap($sub_bundle->getVariationTypeId());
    $used_attributes = [];
    ...
    $view_modes = $this->entityDisplayRepository->getViewModes('commerce_sub_attribute_value');
   ...
   *   The render array.
   */
  protected function getAttributeItemList(FieldItemListInterface $variation_items, SubAttributeInterface $attribute) {
    $build = [
      '#theme' => 'item_list',
    ...
    $view_builder = $this->entityTypeManager->getViewBuilder('commerce_sub_attribute_value');
    /** @var \Drupal\Core\Field\Plugin\Field\FieldType\EntityReferenceItem $variation */
    foreach ($variation_items as $variation) {
      /** @var \Drupal\commerce_sub\Entity\SubAttributeValueInterface $attribute_value */
  ...
  public static function isApplicable(FieldDefinitionInterface $field_definition) {
    ...
    return $entity_type == 'commerce_sub' && $field_name == 'variations';
  }
