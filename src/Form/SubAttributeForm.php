...
class SubAttributeForm extends BundleEntityFormBase {
  ...
  /**
   * Constructs a new SubAttributeForm object.
   *
   * @param \Drupal\commerce_sub\SubAttributeFieldManagerInterface $attribute_field_manager
   *   The attribute field manager.
   */
  public function __construct(SubAttributeFieldManagerInterface $attribute_field_manager) {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $form['id'] = [
      ...
      '#machine_name' => [
        'exists' => '\Drupal\commerce_sub\Entity\SubAttribute::load',
      ],
    ...
    $form['elementType'] = [
      ...
      '#options' => [
        ...
        'commerce_sub_rendered_attribute' => $this->t('Rendered attribute'),
      ],
      '#default_value' => $attribute->getElementType(),
    ];

    // Allow the attribute to be assigned to a sub variaton type.
    ...
    $form['variation_types'] = [
      '#type' => 'checkboxes',
      '#title' => $this->t('Sub variation types'),
    ];
    $attribute_field_map = $this->attributeFieldManager->getFieldMap();
    $variation_type_storage = $this->entityTypeManager->getStorage('commerce_sub_variation_type');
    ...
    if ($this->moduleHandler->moduleExists('content_translation')) {
      ...
      if (!$attribute->isNew()) {
        ...
        $enabled = $translation_manager->isEnabled('commerce_sub_attribute_value', $attribute->id());
      }
  ...
  public function buildValuesForm(array $form, FormStateInterface $form_state) {
    ...
    $wrapper_id = Html::getUniqueId('sub-attribute-values-ajax-wrapper');
    $form['values'] = [
      '#type' => 'table',
      '#header' => [
      ...
      '#tabledrag' => [
        [
          ...
          'group' => 'sub-attribute-value-order-weight',
        ],
      ],
      '#weight' => 5,
    ...
    foreach ($value_map as $index => $id) {
      ...
      $value_form['entity'] = [
        ...
        '#entity_type' => 'commerce_sub_attribute_value',
        '#bundle' => $attribute->id(),
      ...
      $value_form['weight'] = [
        ...
        '#attributes' => [
          'class' => ['sub-attribute-value-order-weight'],
        ],
      ];
    ...
    $access_handler = $this->entityTypeManager->getAccessControlHandler('commerce_sub_attribute_value');
  ...
  protected function saveValues(array $form, FormStateInterface $form_state) {
    ...
    if (!empty($delete_queue)) {
      $value_storage = $this->entityTypeManager->getStorage('commerce_sub_attribute_value');
    ...
    foreach ($form_state->getValue(['values']) as $index => $value_data) {
      /** @var \Drupal\commerce_sub\Entity\SubAttributeValueInterface $value */
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($this->moduleHandler->moduleExists('content_translation')) {
      ...
      if ($translation_manager->isEnabled('commerce_sub_attribute_value', $this->entity->id()) != $enabled) {
        $translation_manager->setEnabled('commerce_sub_attribute_value', $this->entity->id(), $enabled);
    ...
    if ($status == SAVED_NEW) {
      drupal_set_message($this->t('Created the %label sub attribute.', ['%label' => $this->entity->label()]));
      // Send the user to the edit form to create the attribute values.
      $form_state->setRedirectUrl($this->entity->toUrl('edit-form'));
    }
    else {
      $this->saveValues($form, $form_state);
      drupal_set_message($this->t('Updated the %label sub attribute.', ['%label' => $this->entity->label()]));
