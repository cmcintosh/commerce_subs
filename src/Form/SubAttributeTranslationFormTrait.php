...
/**
 * Provides common functionality for the sub attribute translation forms.
 */
trait SubAttributeTranslationFormTrait {
  ...
  /**
   * Builds the translation form for sub attribute values.
   *
   * @param array $form
   ...
   */
  protected function buildValuesForm(array $form, FormStateInterface $form_state, SubAttributeInterface $attribute) {
  ...
  /**
   * Renders the given sub attribute value in the original language.
   *
   * Skips non-translatable fields. Skips all base fields other than the name.
   *
   * @param \Drupal\commerce_sub\Entity\SubAttributeValueInterface $value
   *   The sub attribute value.
   *
   * @return array
   *   The render array.
   */
  protected function renderOriginalValue(SubAttributeValueInterface $value) {
    $value = $value->getUntranslated();
    $view_builder = $this->getEntityTypeManager()->getViewBuilder('commerce_sub_variation');
    $build = [];
