...
/**
 * Defines the inline form for sub variations.
 */
class SubVariationInlineForm extends EntityInlineForm {
  ...
  public function getEntityLabel(EntityInterface $entity) {
    /** @var \Drupal\commerce_sub\Entity\SubVariationInterface $entity */
    $variation_type = $this->loadVariationType($entity->bundle());
    if (!$variation_type->shouldGenerateTitle()) {
      return $entity->label();
    }

    // The generated variation title includes the sub title, which isn't
  ...
  /**
   * Loads and returns a sub variation type with the given ID.
   *
   * @param string $variation_type_id
   *   The variation type ID.
   *
   * @return \Drupal\commerce_sub\Entity\SubVariationTypeInterface
   *   The loaded sub variation type.
   */
  protected function loadVariationType($variation_type_id) {
    if (!isset($this->variationTypes[$variation_type_id])) {
      $storage = $this->entityTypeManager->getStorage('commerce_sub_variation_type');
