...
/**
 * Default implementation of the SubAttributeFieldManagerInterface.
 */
class SubAttributeFieldManager implements SubAttributeFieldManagerInterface {
  ...
  /**
   * Constructs a new SubAttributeFieldManager object.
   *
  ...
  public function getFieldDefinitions($variation_type_id) {
    if (!isset($this->fieldDefinitions[$variation_type_id])) {
      $definitions = $this->entityFieldManager->getFieldDefinitions('commerce_sub_variation', $variation_type_id);
      $definitions = array_filter($definitions, function ($definition) {
        /** @var \Drupal\Core\Field\FieldDefinitionInterface $definition */
        $field_type = $definition->getType();
        $target_type = $definition->getSetting('target_type');
        return $field_type == 'entity_reference' && $target_type == 'commerce_sub_attribute_value';
      });
  ...
  public function getFieldMap($variation_type_id = NULL) {
    if (!isset($this->fieldMap)) {
      if ($cached_map = $this->cache->get('commerce_sub.attribute_field_map')) {
        $this->fieldMap = $cached_map->data;
      }
      else {
        $this->fieldMap = $this->buildFieldMap();
        $this->cache->set('commerce_sub.attribute_field_map', $this->fieldMap);
      }
    }
  ...
  public function clearCaches() {
    ...
    $this->fieldMap = NULL;
    $this->cache->delete('commerce_sub.attribute_field_map');
  }
  ...
  protected function buildFieldMap() {
    $field_map = [];
    $bundle_info = $this->entityTypeBundleInfo->getBundleInfo('commerce_sub_variation');
    foreach (array_keys($bundle_info) as $bundle) {
      /** @var \Drupal\Core\Entity\Display\EntityFormDisplayInterface $form_display */
      $form_display = commerce_get_entity_display('commerce_sub_variation', $bundle, 'form');
  ...
  public function createField(SubAttributeInterface $attribute, $variation_type_id) {
    ...
    $field_storage = FieldStorageConfig::loadByName('commerce_sub_variation', $field_name);
    $field = FieldConfig::loadByName('commerce_sub_variation', $variation_type_id, $field_name);
    if (empty($field_storage)) {
      $field_storage = FieldStorageConfig::create([
        ...
        'cardinality' => 1,
        'settings' => [
          'target_type' => 'commerce_sub_attribute_value',
        ],
        'translatable' => FALSE,
      ]);
      $field_storage->save();
    ...
    if (empty($field)) {
      ...
      /** @var \Drupal\Core\Entity\Display\EntityFormDisplayInterface $form_display */
      $form_display = commerce_get_entity_display('commerce_sub_variation', $variation_type_id, 'form');
  ...
  public function canDeleteField(SubAttributeInterface $attribute, $variation_type_id) {
    ...
    $query = $this->queryFactory->get('commerce_sub_variation')
   ...
   *   The field name.
   */
  protected function buildFieldName(SubAttributeInterface $attribute) {
