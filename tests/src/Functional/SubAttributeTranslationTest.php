...
/**
 * Tests translating sub attributes and their values.
 *
 * @group commerce
 */
class SubAttributeTranslationTest extends SubBrowserTestBase {
  ...
  /**
   * Tests sub attribute translation.
   */
  public function testSubAttributeTranslation() {
    ...
    $this->drupalGet('admin/commerce/sub-attributes/manage/color/translate/fr/add');
    $this->assertSession()->pageTextContains('Label');
    $this->assertSession()->pageTextNotContains('Value');

    // Add two attribute values.
    $red_value = $this->createEntity('commerce_sub_attribute_value', [
      'attribute' => 'color',
      'name' => 'Red',
      'weight' => 0,
    ]);
    $blue_value = $this->createEntity('commerce_sub_attribute_value', [
      'attribute' => 'color',
      'name' => 'Blue',
      'weight' => 1,
    ]);
    ...
    $edit = [
      'translation[config_names][commerce_sub.commerce_sub_attribute.color][label]' => 'Couleur',
    ...
    \Drupal::entityTypeManager()->getStorage('commerce_sub_attribute')->resetCache();
    \Drupal::entityTypeManager()->getStorage('commerce_sub_attribute_value')->resetCache();
    $color_attribute = SubAttribute::load('color');
